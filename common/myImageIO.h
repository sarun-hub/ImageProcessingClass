...
class myImageData {
  ...
  public:
	myImageData(void) { this->mData = NULL; };
	...
	double get(int x, int y) { return this->mData[y * (this->mW) + x]; }
	...
	RGB get_RGB(int x, int y) {
		// if not RGB channel, return grey scale value
		if (this->mCH != 3) {
			throw std::runtime_error(
				"RGB values are not valid: image's CH must have size 3");
		}

		return RGB{this->get(x, y, 0), this->get(x, y, 1), this->get(x, y, 2)};
	}
	...
	void set(int x, int y, RGB value) {
		this->mData[this->mCH * (y * (this->mW) + x) + 0] = value.R;
		this->mData[this->mCH * (y * (this->mW) + x) + 1] = value.G;
		this->mData[this->mCH * (y * (this->mW) + x) + 2] = value.B;
	}
